...
public abstract class ChessGame {
    ...
    public boolean isPawnPromoted(Square fromSquare, Square toSquare) {

        Piece fromPiece = getPieceAtSquare(fromSquare);
        if (fromPiece instanceof Pawn) {
            BoardRank toSquareRank = toSquare.getRank();
            if (fromPiece.getOwner() == Player.BLACK && toSquareRank == BoardRank.FIRST || toSquareRank == BoardRank.EIGHTH && fromPiece.getOwner() == Player.WHITE) {
                return true;
            }
        }
        return false;
